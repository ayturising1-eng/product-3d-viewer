(function () {
  ...
  function normalizePlacement(placement, fallbackType) {
    ...
    const type = requestedType === 'guillotine' ? 'guillotine' : (requestedType === 'zip' ? 'zip' : (requestedType === 'fixed' ? 'fixed' : 'sliding'));
  ...
  function openProductDialog(zone, zones) {
    ...
    if (bulkProductZones) {
      ...
      else $(ids.productZoneInfo).textContent = 'Sürme, giyotin, Sabit Doğrama veya ön katman Zip Perde yerleştirin.';
    }
  ...
  function clearProducts() {
    ...
    if (!count) return;
    if (!window.confirm('Yerleştirilmiş tüm sürme, giyotin, Zip Perde ve Sabit Doğrama ürünleri silinsin mi?')) return;
  ...
  function buildViewerHtml({ width, depth, height, lamellaCount, orientations, postSections, beamSection, placements, zipPlacements, facadeProfiles, cameraState, selectedZoneId: activeZoneId, dimensionVisibility: showDimensionVisibility, productsOpen, productOpenStates, panelStates, panelMasterOpen, toolboxSelectionMode: activeSelectionMode, toolboxSelectionKeys: activeSelectionKeys }) {
    const W = width;
    const D = depth;
    const H = height;
...
function addDividerProfileMesh(base,profile,opts){
  ...
  const height=vertical?Math.max(200,H-220):face;
  const geo=vertical
