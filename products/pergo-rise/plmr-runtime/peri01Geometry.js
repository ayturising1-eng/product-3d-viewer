(function (root) {
  ...
  function normalizeZipScreenPlacement(item, index = 0) {
    ...
    let type = String(raw.type || allowed[0]).trim().toUpperCase();
  ...
  function buildZipScreenBlockDefinition(placement) {
    ...
    const entities = [
      { type: 'polyline', layer, closed: true, points: [[0, 0], [width, 0], [width, height], [0, height]] },
      { type: 'polyline', layer, closed: true, points: [[0, height - boxH], [width, height - boxH], [width, height], [0, height]] },
      { type: 'polyline', layer, closed: true, points: [[0, 0], [guideW, 0], [guideW, fabricTop], [0, fabricTop]] },
      { type: 'polyline', layer, closed: true, points: [[width - guideW, 0], [width, 0], [width, fabricTop], [width - guideW, fabricTop]] },
      { type: 'polyline', layer, closed: true, points: [[guideW, 0], [width - guideW, 0], [width - guideW, bottomBarH], [guideW, bottomBarH]] },
      { type: 'hatch', layer: 'HATCH_FABRIC', color: 42, points: [[fabricLeft, fabricBottom], [fabricRight, fabricBottom], [fabricRight, fabricTop], [fabricLeft, fabricTop]], patternKind: 'screen' }
    ];
    const motorRight = String(placement.motorDirection || 'RIGHT').toUpperCase() !== 'LEFT';
    const textH = Math.max(18, Math.min(42, boxH * 0.28));
    entities.push({ type: 'text', layer, x: motorRight ? width - 22 : 22, y: height - boxH / 2, height: textH, value: 'MOTOR', align: motorRight ? 'right' : 'left', rotation: 0 });
    const cableText = String(placement.cableExitDirection || 'REAR').toUpperCase();
    entities.push({ type: 'text', layer, x: width / 2, y: height - boxH / 2, height: Math.max(14, textH * 0.72), value: cableText, align: 'center', rotation: 0 });
